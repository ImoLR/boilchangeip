...
- 后台定时换 IP 在换 IP 前查询当前 IP 失败时，不再直接结束；会按 5 分钟、30 分钟、1 小时进行三次后台重试。
- 重试只针对失败的单台 VPS，成功获取当前 IP 后继续该 VPS 的换 IP 流程，不会重新执行整组定时任务。
- Timer 与后台重试共用串行执行锁，避免后台任务并发触发 `changeIP`。
- 手动 `/change` 的 Drop-on-Busy 行为保持不变。

## v2.2.3 - 2026-07-26

### 改进

- 简化首次安装流程，安装阶段只配置 Telegram Bot Token，服务器通过 Telegram 向导添加。
- 添加服务器时自动生成内部 server id，用户不再需要手工填写本地标识。
- 新增服务器时先验证 Token，再填写名称和可选地址，降低首次配置成本。
...
- 修复 `update.sh` 依赖本地长期源码目录，导致普通安装用户无法更新的问题。
- `install.sh`、`update.sh` 和 `uninstall.sh` 现在均可从任意工作目录独立执行。
- 安装、更新和卸载流程不再依赖调用者当前目录。
- `update.sh` 改为使用临时目录下载源码和构建，不再要求本地源码目录存在。
...
- boilchangeip 已从 Fork 脱离，作为独立项目维护。
